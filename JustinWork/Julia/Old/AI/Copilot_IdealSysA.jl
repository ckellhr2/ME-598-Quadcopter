...
function get_state_from_bullet(body_id)
    pos, quat = #EDITME_getBasePositionAndOrientation(body_id)
    vel, ang_vel = #EDITME_getBaseVelocity(body_id)
    roll, pitch, yaw = #EDITME_getEulerFromQuaternion(quat)
...
function get_total_mass(body_id)
    total_mass = 0.0
    num_joints = #EDITME_getNumJoints(body_id)

    for link_index in -1:(num_joints - 1)
        dyn = #EDITME_getDynamicsInfo(body_id, link_index)
        total_mass += dyn[1]
    end
...
function keep_gui_alive(dt = 1/240)
    while #EDITME_isConnected()
        #EDITME_stepSimulation()
        sleep(dt)
    end
end
...
function run_simulation(; 
    ...
    try
        #EDITME_resetSimulation()
        #EDITME_setAdditionalSearchPath(#EDITME_dataPath())
        #EDITME_setGravity(0, 0, -g)
        #EDITME_loadURDF("plane.urdf")

        script_dir = @__DIR__
        urdf_path = joinpath(script_dir, "quadrotor", "quadrotor.urdf")
        quad_id = #EDITME_loadURDF(urdf_path, start_pos)

        m = get_total_mass(quad_id)
        inertia_diag = #EDITME_getDynamicsInfo(quad_id, -1)[3]
        ...
        verbose && println("Simulation started.")
        ...
        while step < max_steps
            ...
            if verbose && log_interval > 0 && step % log_interval == 0
                println("t=$(step*dt)s pos=$(x[1:3]) vel=$(x[4:6]) thrust=$(u)")
            end
            ...
            if pos_error < 0.1 && vertical_speed < 0.15 && step > 300
                verbose && println("Reached goal!")
                break
            end
        end

        verbose && step >= max_steps && println("Max simulation time reached.")
        ...
        result = Dict(
            ...
            "goal_z" => x_goal_pos[3],
            "steps" => step,
            "reached_goal" => (norm(final_state[1:3] .- x_goal[1:3]) < 0.1 && abs(final_state[6]) < 0.15),
        ...
        if keep_alive && connection_mode == #EDITME_GUI
        ...
        keep_alive && connection_mode == #EDITME_GUI && keep_gui_alive()
        rethrow()
    finally
        if physics_client !== nothing #&& #EDITME_isConnected(physics_client)
            #EDITME_disconnect(physics_client)
        end
    end
end
...
# Apply rotor forces
# ==========================
function apply_motor_forces(body_id, u)
    ...
    for i in 1:4
        ...
        #EDITME_applyExternalForce(
        #    objectUniqueId = body_id,
        #    linkIndex = -1,
        #    forceObj = [0.0, 0.0, f],
        #    posObj = collect(r_body),
        #    flags = #EDITME_LINK_FRAME
        #)
    end

    yaw_torque = k_yaw * dot(yaw_signs, u)

    #EDITME_applyExternalTorque(
        #objectUniqueId = body_id,
        #linkIndex = -1,
...
function ilqr(x0, x_goal, N, dt, m, inertia_diag, Q, R, Qf, u_init=nothing; max_iter=25)
    ...
    delta_u_seq = u_init === nothing ? zeros(N, NU) :
