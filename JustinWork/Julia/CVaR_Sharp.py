...
def corridor_path_lines(waypoints, resolution):
    path = []
    for i in range(len(waypoints)-1):
        start = waypoints[i]
        end = waypoints[i+1]
        for j in range(resolution):
            point = start + (end - start) * (j / resolution)
            path.append(point)
    path.append(waypoints[-1])  # Ensure the final waypoint is included
...
path = corridor_path_lines(waypoints, resolution)
