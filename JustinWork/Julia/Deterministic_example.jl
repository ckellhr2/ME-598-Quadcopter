...
## SYSTEM SETUP
###################################################################
function setup_system(; Ntraj=tjs) # Ntraj = number of trajectories for ensemble sims, default val 10
...
## MAIN
###################################################################
function main(; Ntraj = tjs, max_GPUs=10, systems=[:nominal_sys, :true_sys, :L1_sys]) 

    @info "Warmup run for JIT compilation"
    println("=====================================") 
    warmup_setup = setup_system(; Ntraj = Ntraj)
...
function generate_state_plots(; path=joinpath(@__DIR__, "sol_logs"), max_traj=50max_tjs0)
...
generate_state_plots(; max_traj=max_tjs)