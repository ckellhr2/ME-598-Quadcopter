...
function main(; Ntraj=2, max_GPUs=10, systems=[:nominal_sys, :true_sys, :L1_sys],
...
function log_state_results(setup, solutions; path=joinpath(@__DIR__, "quad_ilqr_sol_logs_J"))
    mkpath(path)
...
function generate_position_plot(; path=joinpath(@__DIR__, "quad_ilqr_sol_logs_J"), max_traj=1000,
...
function generate_nominal_position_plots(; path=joinpath(@__DIR__, "quad_ilqr_sol_logs_J"),
                                         max_traj=1000,
...
function generate_state_plots(; path=joinpath(@__DIR__, "quad_ilqr_sol_logs_J"), max_traj=1000)
    nom = load(joinpath(path, "states_nominal.jld2"));
    tru = load(joinpath(path, "states_true.jld2"));
    L1  = load(joinpath(path, "states_L1.jld2"));
